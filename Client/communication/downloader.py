...
import os
import random
# request: { file_name: , piece_seq_no: }

peers = { 0: ['localhost', 56775] }
torrents = [ {'name': 'main.py', 'piece_size': 128, 'pieces_info': 
						[
...
class Downloader:
	...
	def get_piece(self, seq_num, piece_size, file_size, peer):
		...
		request = json.dumps({ 'file_name': self.file_name, 'piece_seq_no': seq_num, 'piece_size': piece_size, 'file_size': file_size })
	...
	def write_piece(self, seq_num, piece_size, file_size, peer):
		...
		if not os.path.isfile('new'+self.file_name):
			open('new'+self.file_name, 'w').close()

		with open('new'+self.file_name, 'r+b') as f:
			f.seek(start)
			f.write(self.get_piece(seq_num, piece_size, file_size, peer))

	def download(self):
		file_info = next((item for item in torrents if item["name"] == self.file_name), None)
